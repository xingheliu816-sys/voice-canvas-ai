...
在 `.env.local` 中配置 `GLM_API_KEY` 可启用 `/api/voice-command` 的 GLM Function Calling 增强解析（智谱 glm-4），处理更复杂的自然语言指令。旧版 `ANTHROPIC_API_KEY` 接口仍保留为兼容路径。
...
用户语音
  ...
  -> /api/voice-command
     ...
     -> Layer 2: GLM glm-4 Function Calling
...
- 含 `AI`、`智能`、`帮我`、`你觉得`、`随便` 等显式 AI 触发词时，优先调用 GLM。
- 本地规则高置信命中时直接返回，保持基础绘图指令的低延迟。
- GLM 超时、未配置 Key 或网络失败时，进入离线兜底，不阻断基础绘图。
...
| AI 路由 | `/api/voice-command` 三层解析：本地规则、GLM、离线兜底 | ✅ |
| 绘图 | 圆、矩形、三角形、直线、文字 | ✅ |
...
| GLM API | glm-4 | 智谱 AI OpenAI 兼容 Function Calling 指令解析（可选） | 第三方服务 |
