...
interface RouterDeps {
  callDeepSeek?: (request: VoiceCommandRequest) => Promise<DeepSeekToolCall[]>;
  now?: () => number;
}
...
export async function processVoiceCommand(
  ...
  try {
    const toolCalls = await (deps.callDeepSeek ?? callDeepSeek)(request);
  ...
  return {
    ...
    latencyMs: elapsed(startedAt, deps.now),
    warning: 'DeepSeek returned no executable tool calls'
  };
}

export async function callDeepSeek(request: VoiceCommandRequest): Promise<DeepSeekToolCall[]> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`DeepSeek API ${response.status}: ${detail.slice(0, 180)}`);
    }
...
export function convertToolCallsToCommands(toolCalls: DeepSeekToolCall[]): Command[] {
...
function convertToolCallToCommand(call: DeepSeekToolCall): Command | null {
...
function extractAskUserQuestion(toolCalls: DeepSeekToolCall[]): string | undefined {
